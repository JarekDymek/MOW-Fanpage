export function attachFlow(form,getPhotos,isPreparing){
  ...
  const names=['Sposób redakcji','Tekst i wydarzenie','Zdjęcia','Wizerunek','Potwierdzenia'];
  ...
  function update(){
    ready=sections.map((s,i)=>i===2?getPhotos()>0&&!isPreparing():[...s.querySelectorAll('[required]')].every(el=>el.disabled||el.validity.valid&&(!['text','textarea'].includes(el.type)||el.value.trim())));
