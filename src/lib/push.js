...
export async function enablePush(userId){
 ...
 const {error}=await supabase.from('mow_push_subscriptions').upsert({user_id:userId,endpoint:payload.endpoint,p256dh:payload.keys?.p256dh,auth:payload.keys?.auth,user_agent:navigator.userAgent},{onConflict:'user_id,endpoint'});
 if(error) throw error; return {ok:true};
}
export async function notifyWorkflow(eventType,submissionId){
 const {data:{session}}=await supabase.auth.getSession(); if(!session)return;
 await supabase.functions.invoke('mow-notify-workflow',{body:{eventType,submissionId},headers:{Authorization:`Bearer ${session.access_token}`}}).catch(()=>{});
}
