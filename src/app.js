...
export async function startApp(){
  try {
    ...
    function shell(body){
      ...
      document.body.innerHTML=`<header><div class="brand"><img src="/icons/icon.svg" alt=""><b>MOW Fanpage</b>${ENTRY==='admin'?'<span class="pill">ADMIN</span>':''}</div><nav>${st.user&&st.profile?'<button data-home>Start</button><button data-new>Nowy materiał</button><button data-mine>Moje</button><button data-profile>Profil</button>':''}${st.profile?.role==='moderator'?'<button data-mod>Moderator</button>':''}${st.install?'<button data-install>Zainstaluj</button>':''}${st.user?'<button data-out>Wyloguj</button>':''}</nav></header><main>${body}</main>`;
    ...
    function form(){
      ...
      $('#ph').onchange=async e=>{
        ...
        st.preparing=true;$('#pm').textContent='Przygotowywanie…';
        try{const prepared=[];for(const f of fs)prepared.push(await prep(f));st.photos=prepared;thumbs();$('#pm').textContent=`Gotowe: ${st.photos.length}/15`;}
        catch(error){$('#pm').textContent='Nie udało się odczytać zdjęcia. Wybierz plik JPEG lub PNG.';}
        finally{st.preparing=false;}
      };
      $('#f').onsubmit=submit;
    }

    function thumbs(){for(const url of st.thumbUrls||[])URL.revokeObjectURL(url);st.thumbUrls=[];const b=$('#th');b.innerHTML=st.photos.map((f,i)=>`<div class="thumb"><img src="${(()=>{const url=URL.createObjectURL(f);st.thumbUrls.push(url);return url})()}"><button type="button" data-i="${i}">×</button></div>`).join('');b.querySelectorAll('button').forEach(x=>x.onclick=()=>{if(st.upload||st.sending)return;st.photos.splice(+x.dataset.i,1);thumbs()})}
    ...
    async function submit(e){
      e.preventDefault();const m=$('.msg');if(st.sending)return;if(st.preparing)return m.textContent='Poczekaj na przygotowanie zdjęć.';if(!st.photos.length)return m.textContent='Dodaj co najmniej jedno zdjęcie.';
      st.sending=true;const formNode=e.currentTarget,button=formNode.querySelector('button.primary.big');button.disabled=true;
      try{
      ...
      if(!st.upload){const result=await S.from('mow_submissions').insert(p).select().single();if(result.error)throw result.error;st.upload={id:result.data.id,index:0,uploaded:false};}
      const a={data:{id:st.upload.id}};formNode.querySelectorAll('input,textarea').forEach(el=>el.disabled=true);
      for(let i=st.upload.index;i<st.photos.length;i++){
        const x=st.photos[i],path=`${st.user.id}/${a.data.id}/${i}-${x.name}`,u=st.upload.uploaded?{error:null}:await S.storage.from('mow-materials').upload(path,x,{contentType:'image/jpeg'});
        if(u.error)throw u.error;st.upload.uploaded=true;
        ...
        if(d.error)throw d.error;st.upload.index=i+1;st.upload.uploaded=false;
      }
      const r=await S.rpc('mow_submit_submission',{p_submission_id:a.data.id});if(r.error)throw r.error;
      await notify('submitted',a.data.id);location.href=`${ENTRY==='admin'?'/admin/':'/wychowawca/'}?submission=${a.data.id}`;
      }catch(error){m.textContent='Wysyłanie przerwane: '+error.message+(st.upload?' Część materiału jest zapisana. Naciśnij Wyślij materiał, aby wznowić.':'');}
      finally{st.sending=false;button.disabled=false;}
    }
